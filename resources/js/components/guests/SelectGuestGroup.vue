...
import { change } from '@/actions/App/Http/Controllers/Admin/GuestGroupController';
...
const groups = computed<SelectableGroup[]>(
    () => usePage().props.selectableGroups,
);
const currentGroup = computed(() =>
    groups.value.find((g) => g.id === props.guest.group_id),
);

const open = ref(false);
const submit = () =>
    form.submit(
        change({ guest: props.guest.id, group: selectedGroup.value.id }),
        {
            preserveScroll: true,
        ...
        <DialogContent>
            ...
            <form @submit.prevent="submit" class="space-y-4">
                <div class="grid gap-2 py-2">
                    <ComboboxRoot class="relative" v-model="selectedGroup">
                        ...
                        <ComboboxContent
                            position="popper"
                            :side-offset="4"
                            class="z-50 w-(--reka-popper-anchor-width) overflow-hidden rounded-[10px] border border-border/60 bg-popover text-popover-foreground shadow-md data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0"
                        >
                            <ComboboxViewport
                            ...
                            >
                                ...
                                <ComboboxItem
                                    ...
                                    class="relative flex w-full cursor-pointer items-center rounded-sm px-2 py-1.5 text-sm outline-none select-none data-disabled:opacity-60 data-disabled:hover:cursor-not-allowed data-highlighted:bg-accent data-highlighted:text-accent-foreground"
                                >
                                    <ComboboxItemIndicator
                ...
                <DialogFooter class="gap-2">
                    ...
                    <Button
                        type="submit"
                        :disabled="form.processing || !selectedGroup?.id"
                    >
                        Change group
                    </Button>
                </DialogFooter>
