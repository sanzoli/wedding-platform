 ...
 * the component tree stays untouched.
 */
...
function guest(
    ...
    nameStatus: GuestNameStatus = 'known',
): Guest {
    return {
        id,
        name,
...
function group(id: string, primaryGuestId: string): GuestGroup {
    return {
        id,
        wedding_id: WEDDING,
...
export const mockGuests: Guest[] = [
    ...
    // 2. Sarah & James Mitchell — couple EN
    guest(
        'g_002',
        'gg_02',
        true,
    ...
    guest('g_003', 'gg_02', false, 'James', 'Mitchell', 'en'),
    ...
    // 5. Camila & Mateo Restrepo — couple ES
    guest(
        'g_010',
        'gg_05',
        true,
    ...
    guest('g_011', 'gg_05', false, 'Mateo', 'Restrepo', 'es'),
...
/**
 * Compose the view model the table consumes from the parallel arrays.
 * Filters archived guests so soft-deletes drop out naturally.
 */
export function buildGuestGroupViews(
    groups: GuestGroup[] = mockGuestGroups,
    guests: Guest[] = mockGuests,
): GuestGroupView[] {
    const active = guests.filter((g) => g.archived_at === null);

    return groups.map((grp) => {
        const members = active.filter((g) => g.guest_group_id === grp.id);
        const primary = members.find((g) => g.id === grp.primary_guest_id);
        ...
        const companions = members.filter((g) => !g.is_primary);
        const pendingCompanions = companions.filter(
            (g) => g.name_status === 'pending',
        ).length;
